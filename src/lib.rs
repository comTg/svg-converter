...
impl SvgConverter {
    ...
    pub fn svg_to_png(&mut self, svg_data: &str, width: u32, height: u32) -> Result<String, JsValue> {
        match self.convert_svg_to_png(svg_data, width, height) {
            Ok(png_data) => {
    ...
    pub fn png_to_svg(&mut self, png_data_base64: &str, simplify: u8) -> Result<String, JsValue> {
        // 从Base64解码PNG数据
        let png_data = extract_base64_data(png_data_base64)
            .map_err(|e| JsValue::from_str(&format!("Invalid base64 data: {}", e)))?;
        
        match self.convert_png_to_svg(&png_data, simplify) {
            Ok(svg_data) => {
...
impl SvgConverter {
    // 内部SVG到PNG转换方法
    fn convert_svg_to_png(&self, svg_data: &str, width: u32, height: u32) -> Result<Vec<u8>> {
        // 配置SVG解析选项
    ...
    // 内部PNG到SVG转换方法
    fn convert_png_to_svg(&self, png_data: &[u8], simplify: u8) -> Result<String> {
        // 读取输入PNG数据
...
fn extract_base64_data(data_url: &str) -> Result<Vec<u8>> {
